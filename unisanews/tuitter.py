...
class Tuitter(object):
    ...
    def tweet_news(self, news_items):
        for news_item in news_items:
            ...
            if self._today_news_item(news_item):
                ...
                self._send_tweet(news_item, current_app.config["TWEET_SENDING_EMULATION"])
    ...
    def _send_tweet(self, item, emulate):
        ...
        if emulate:
            ...
            auth.set_access_token(current_app.config["TWITTER_ACCESS_TOKEN"],
                                  current_app.config["TWITTER_TOKEN_SECRET"])
            api = tweepy.API(auth)
